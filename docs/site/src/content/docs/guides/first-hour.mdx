...
Six of them, one at a time. There is no wrong answer you cannot change later —
...
| Your name and e-mail?   | They become the owner account — you — and the address your sign-in link is made for.                                |
...
machine, makes you its owner, puts some invented data in it, and starts the app.
It also downloads, once, the browser it takes screenshots with. You do not type
any of those commands; it runs `pnpm install`,
`pnpm exec playwright install chromium`,
`pnpm db:seed --owner you@example.com "Your Name" --sample` with your own address
and name, and `pnpm dev`, and tells you what happened.

> **You should see:** a line naming the account it made for you, a sign-in link
> the agent shows you, and the app running on port 5173.
...
Open the sign-in link from the previous step. It works once, within the hour; if
it has expired, ask the agent for a new one.
