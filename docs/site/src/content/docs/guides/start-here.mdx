...
Where that hour sits in the whole arc, so you know what is not in it:

```mermaid
flowchart TB
  A["Hour one<br/>the app runs on your laptop<br/>and on your phone"]
  B["The weeks after<br/>one to a few screens a week,<br/>each one described, built, tried"]
  C["The day other people come in<br/>a server, a domain, e-mail<br/>— an afternoon, once"]
  D["From then on<br/>one command puts a change live"]
  A --> B --> C --> D
  D -->|"the next thing you need"| B
```
