...
```mermaid
flowchart LR
  Laptop["Your laptop<br/>pnpm release &lt;commit&gt;"]
  subgraph Server["One rented server"]
    direction TB
    Caddy["Caddy<br/>the front door, handles HTTPS"]
    App["The app"]
    DB["PostgreSQL<br/>every record you have"]
    Backup["Nightly copy<br/>03:15, kept 14 days"]
    Caddy --> App --> DB
    DB -.-> Backup
  end
  People["The people who use it"] --> Caddy
  Laptop -->|"over SSH"| Server
```
