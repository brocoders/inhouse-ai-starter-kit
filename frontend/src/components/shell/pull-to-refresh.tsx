 ...
 * does on a desktop.
 *
 * It is written here rather than taken from a package. The maintained ones are
 * either a styled widget that would have to be re-skinned to match the design
 * system, or a bare hook about the size of this file; neither knows that this
...
function pageIsLocked(target: Element | null) {
  ...
  if (getComputedStyle(document.body).overflow === 'hidden') return true;
  // A list that scrolls sideways owns its own gestures.
...
export function PullToRefresh() {
  ...
  useEffect(() => {
    ...
    const place = (distance: number, animate: boolean, opacity?: number) => {
      ...
      node.style.transition = animate ? 'transform 200ms ease-out, opacity 200ms ease-out' : 'none';
        ...
        <div className="flex size-9 items-center justify-center rounded-full border bg-background shadow-md">
          {phase === 'done' ? (
            ...
            <RefreshCw
              ...
              style={
                phase === 'refreshing' ? undefined : { transform: 'rotate(var(--pull-turn, 0deg))' }
              }
            />
          )}
        </div>
