...
/** A row of labelled controls above a list or a chart. */
export function FilterBar({
  ...
  return (
    <div
      id={id}
      className={cn('flex flex-wrap items-end gap-4 rounded-xl border bg-card p-4', className)}
    >
      {children}
    </div>
  );
...
export function Field({
   ...
   * 1 Jan – 22 Sept 2026" does not need the word "Period" above it. A stacked
   * label costs about 24px of height each, which is what pushes the first row
