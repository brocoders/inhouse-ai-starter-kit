...
/**
 * A long list that stays light: only the rows in view are in the document, the
 * page scrolls as one, and the next page is asked for when the end comes near.
 * Rows measure themselves, so a row with two lines of text takes the room it
 * needs. A "Show more" button remains for anyone whose browser does not report
 * the scroll position.
 */
export function PagedList<T>({
  ...
  footer?: ReactNode;
  /** What a screen reader calls this list while it loads. */
  label?: string;
}) {
  ...
  if (loading)
    return (
      <div role="status" aria-label={label ?? t('list.loading')} className="space-y-3">
        {[0, 1, 2, 3].map((n) => (
  ...
  return (
    <div className="space-y-3">
      <Card className="gap-0 py-0 shadow-xs">
        <CardContent ref={listRef} className="px-0">
      ...
      {hasMore && (
        <Button variant="outline" className="w-full" disabled={loadingMore} onClick={onLoadMore}>
          {loadingMore ? t('action.loadingMore') : t('action.showMore')}
        </Button>
      )}
